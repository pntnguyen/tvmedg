...
```{r}
cal_ci <- function(data,ci = 0.95){
  qnt <- quantile(data, probs = c((1-ci)/2,1 - (1-ci)/2))
  paste0("(",round(qnt[1],3),",",round(qnt[2],3),")")
}
```
...
tvmedg <- function(data,
                   ...
                   lreg = c("binomial","gaussian","gaussian"),
                   yreg = "binomial",dof = 3,
                   montecarlo = 10000,length = 12,parallel=TRUE,iter = 1,ci = .95){
  ...
  ## print result
  cat("Q(1,1):", round(mean(qqq$mQ11), 3),cal_ci(qqq$mQ11,ci),'\n')
  cat("Q(1,0):", round(mean(qqq$mQ10), 3),cal_ci(qqq$mQ10,ci),'\n')
  cat("Q(0,0):", round(mean(qqq$mQ00), 3),cal_ci(qqq$mQ00,ci),'\n')
  
  cat("Indirect:", round(mean(qqq$rIE_b), 3),cal_ci(qqq$rIE_b,ci),'\n')
  cat("Direct:", round(mean(qqq$rDE_b), 3),cal_ci(qqq$rDE_b,ci),'\n')
  cat("Total:", round(mean(qqq$rTE_b), 3),cal_ci(qqq$rTE_b,ci),'\n')
  cat("Proportional explain:", 
      round(mean(qqq$rPE_b), 3),cal_ci(qqq$rPE_b,ci),'\n')
